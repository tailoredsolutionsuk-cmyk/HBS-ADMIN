...
export default function AdminPage() {
  ...
  const filteredWebsites = useMemo(() => websites.filter((site) => `${site.name} ${site.domain} ${site.type}`.toLowerCase().includes(query.toLowerCase())), [query]);
    ...
    <main className="admin-shell">
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav" aria-label="Admin navigation">
          {navItems.map((item) => <button key={item.label} className={`admin-nav-item ${activeNav === item.label ? "active" : ""}`} onClick={() => setActiveNav(item.label)}><Icon name={item.icon} />{item.label}{item.label === "Websites" && <span className="admin-nav-count">{websites.length}</span>}</button>)}
        </nav>
      ...
      <section className="admin-content">
        <header className="admin-topbar">
          ...
          <div className="admin-topbar-actions"><span className="admin-saved"><i />All systems operational</span><button className="admin-outline-button" onClick={() => showNotice("Client dashboard opened in a new tab")}><Icon name="external" size={14} />Client dashboard</button><button className="admin-avatar-button">JM</button></div>
        </header>
        ...
        <div className="admin-main">
          {activeNav === "Overview" ? <>
            ...
            <section className="admin-stats" aria-label="Workspace statistics">
              <article className="admin-stat-card"><span className="admin-stat-icon peach"><Icon name="globe" size={17} /></span><span className="admin-stat-label">Active websites</span><strong>6</strong><small><b>+2</b> this month</small></article>
              <article className="admin-stat-card"><span className="admin-stat-icon blue"><Icon name="layers" size={17} /></span><span className="admin-stat-label">Deployments</span><strong>14</strong><small><b>+18%</b> from last month</small></article>
              <article className="admin-stat-card"><span className="admin-stat-icon lavender"><Icon name="users" size={17} /></span><span className="admin-stat-label">Team members</span><strong>3</strong><small>Across <b>6</b> websites</small></article>
              <article className="admin-stat-card"><span className="admin-stat-icon mint"><Icon name="activity" size={17} /></span><span className="admin-stat-label">Uptime</span><strong>99.9%</strong><small><b>Healthy</b> this month</small></article>
            </section>
