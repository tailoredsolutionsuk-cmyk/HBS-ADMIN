...
export async function GET() {
  ...
  const [clients, leads, tasks, notes, activities] = await Promise.all([
    ...
    supabase.from("leads").select("id,business_name,contact_name,name,email,phone,status,source,project_type,estimated_value,assigned_to,notes,help_needed,created_at,updated_at").order("created_at", { ascending: false }),
  ...
  return NextResponse.json({
    ...
    metrics: {
      ...
      openLeads: leadRows.filter((lead) => openStatuses.has(String(lead.status || "new").toLowerCase())).length,
      pipelineValue: leadRows.reduce((total, lead) => total + Number(lead.estimated_value || 0), 0),
